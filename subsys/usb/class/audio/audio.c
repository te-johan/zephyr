...
static void fix_format_descriptors(struct usb_if_descriptor *iface)
{
	...
	fmt_desc = (struct format_type_i_descriptor *)
		((u8_t *)if_desc + USB_PASSIVE_IF_DESC_SIZE +
		 USB_AC_CS_IF_DESC_SIZE);

	for (int i = 0; i < fmt_desc->bSamFreqType; i++) {
		uint32_t *freq = audio_dev_data->frequencies[0];
		fmt_desc->tSamFreq[3*i+0] = freq[i] & 0xff;
		fmt_desc->tSamFreq[3*i+1] = (freq[i] >> 8) & 0xff;
		fmt_desc->tSamFreq[3*i+2] = (freq[i] >> 16) & 0xff;
	}
 ...
 *
 * @param [in] audio_dev_data USB audio device data.
 * @param [in] endpoint       USB Audio endpoint number.
 *
 ...
 * @return audio_dev_data for given interface, NULL if not found.
 */
static struct usb_audio_dev_data *get_audio_dev_data_by_endpoint(u8_t ep,
								 u8_t *device)
{
	struct usb_dev_data *dev_data;
	struct usb_audio_dev_data *audio_dev_data;
...
static int handle_sampling_freq_req(struct usb_audio_dev_data *audio_dev_data,
				    struct usb_setup_packet *pSetup,
				    s32_t *len, u8_t **data, u8_t ep,
				    u8_t device)
{
	u8_t *frequency = audio_dev_data->frequency[device];

	switch (pSetup->bRequest) {
	case USB_AUDIO_SET_CUR:
		memcpy(frequency, *data, 3);
		break;
	case USB_AUDIO_GET_CUR:
		memcpy(*data, frequency, 3);
		break;
	default:
		return -EINVAL;
	}
	...
	if (audio_dev_data->ops &&
		...
		if (pSetup->bRequest == USB_AUDIO_SET_CUR) {
			...
			audio_dev_data->ops->sampling_freq_update_cb(
				audio_dev_data->common.dev, dir, freq);
		}
	}
			       ...
			       u8_t **data)
{
	...
	u8_t ep = (pSetup->wIndex) & 0xFF;
	u8_t cs = ((pSetup->wValue) >> 8) & 0xFF;

	audio_dev_data = get_audio_dev_data_by_endpoint(ep, &device);
	if (audio_dev_data == NULL) {
		LOG_INF("audio_dev_data is NULL");
		return -EINVAL;
	}
