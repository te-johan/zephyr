...
#define USB_FORMAT_TYPE_I_DESC_SIZE(iface)				\
	((struct format_type_i_descriptor *)((u8_t *)iface +		\
...
#define DECLARE_FORMAT_DESCRIPTOR(dev, i, freq_cnt)	\
...
DECLARE_FORMAT_DESCRIPTOR(dev, i, GET_FREQ_CNT(dev, i));		\
struct dev##_descriptor_##i {						\
	...
	struct usb_if_descriptor as_interface_alt_1;			\
		...
		struct dev##_format_type_i_descriptor format;		\
...
DECLARE_FORMAT_DESCRIPTOR(dev##_MIC, i, GET_FREQ_CNT(dev##_MIC, i));	\
DECLARE_FORMAT_DESCRIPTOR(dev##_HP, i, GET_FREQ_CNT(dev##_HP, i));	\
struct dev##_descriptor_##i {						\
	USB_AUDIO_IAD_DECLARE						\
	struct usb_if_descriptor std_ac_interface;			\
	struct dev##_cs_ac_if_descriptor_##i cs_ac_interface;		\
